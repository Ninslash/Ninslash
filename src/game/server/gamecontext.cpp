...
void CGameContext::OnMessage(int MsgID, CUnpacker *pUnpacker, int ClientID)
{
	...
	if (Server()->ClientIngame(ClientID))
	{
		if (MsgID == NETMSGTYPE_CL_SAY)
		{
			...
			{
				char aBuf[128];
				str_format(aBuf, sizeof(aBuf), "Body: {%d}, feet: {%d}, skin: {%d}, topper: {%d}", pPlayer->m_TeeInfos.m_ColorBody, pPlayer->m_TeeInfos.m_ColorFeet, pPlayer->m_TeeInfos.m_ColorSkin, pPlayer->m_TeeInfos.m_ColorTopper);
			...
			{
				char aBuf[128];
				str_format(aBuf, sizeof(aBuf), "Mapgen seed: {%d}", g_Config.m_SvMapGenSeed);
				SendChatTarget(ClientID, aBuf);
				SkipSending = true;
			}
			...
			{
				char aBuf[128];
				str_format(aBuf, sizeof(aBuf), "Highest level reached on server: {%d}", Server()->GetHighScore());
				SendChatTarget(ClientID, aBuf);
				SkipSending = true;
			}
			...
			{
				char aBuf[128];
				str_format(aBuf, sizeof(aBuf), "Number of player profiles in Invasion: {%d}", Server()->GetPlayerCount());
				SendChatTarget(ClientID, aBuf);
				SkipSending = true;
			}
			...
			{
				char aBuf[128];
				str_format(aBuf, sizeof(aBuf), "Number of weapon objects: {%d}", m_World.FindEntities(vec2(0, 0), 0.0f, NULL, 99999, CGameWorld::ENTTYPE_WEAPON));
				SendChatTarget(ClientID, aBuf);
				SkipSending = true;
			}
			...
			{
				char aBuf[128];
				str_format(aBuf, sizeof(aBuf), "Number of pickups: {%d}", m_World.FindEntities(vec2(0, 0), 0.0f, NULL, 99999, CGameWorld::ENTTYPE_PICKUP));
				SendChatTarget(ClientID, aBuf);
				SkipSending = true;
			}
			...
			{
				char aBuf[128];
				str_format(aBuf, sizeof(aBuf), "Number of entities: {%d}", m_World.CountEntities());
				SendChatTarget(ClientID, aBuf);
				SkipSending = true;
			}
			...
			if (false && strcmp(pMsg->m_pMessage, "/jumphigh") == 0)
			{
				...
				{
					char aBufVote[128];
					str_format(aBufVote, sizeof(aBufVote), "Jump to level {%d}", pData->m_HighestLevel);
					//char aBufReason[128]; str_format(aBufReason, sizeof(aBufReason), "Server highest: {%d}", pData->GetHighScore(1));
					char aBufCmd[128];
					str_format(aBufCmd, sizeof(aBufCmd), "sv_inv_fails 0; sv_mapgen_level {%d}; sv_mapgen_seed {%d}; sv_map generate_inv2;", pData->m_HighestLevel, pData->m_HighestLevelSeed);
				...
				{
					char aBuf[128];
					str_format(aBuf, sizeof(aBuf), "This is the highest level you have been to.");
					SendChatTarget(ClientID, aBuf);
					SkipSending = true;
				}
			}
		...
		else if (MsgID == NETMSGTYPE_CL_CALLVOTE)
		{
			...
			if (pPlayer->m_LastVoteCall && Timeleft > 0)
			{
				...
				SendChatTarget(ClientID, _("You must wait {%d} seconds before making another vote"), (Timeleft / Server()->TickSpeed()) + 1);
				return;
			}
		...
		else if (MsgID == NETMSGTYPE_CL_SETTEAM && !m_World.m_Paused)
		{
			...
			{
				pPlayer->m_LastSetTeam = Server()->Tick();
				SendBroadcast(-1, _("Teams are locked", ClientID);
				return;
			}
			*/
			...
			if(pPlayer->m_TeamChangeTick > Server()->Tick())
			{
				...
				SendBroadcast(-1, _(aBuf, ClientID);
				return;
			}
		...
		else if (MsgID == NETMSGTYPE_CL_INVENTORYACTION)
		{
			...
			case INVENTORYACTION_COMBINE:
				pPlayer->CombineItem(pMsg->m_Item1, pMsg->m_Item2);
				break;
			case INVENTORYACTION_TAKEPART:
				pPlayer->TakePart(pMsg->m_Item1, pMsg->m_Slot, pMsg->m_Item2);
				break;
...
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	pSelf->SendBroadcast(-1, false, _(pResult->GetString(0)));
}
