...
{
	if (frandom() < 0.2f)
		return GetModularWeapon(SW_GRENADE3, SW_BAZOOKA + rand() % 2);
...
void IGameController::DeathMessage()
{
	...
	{
	case 0:
		GameServer()->SendBroadcast(-1, false, _("All hope is lost"));
		break;
	case 1:
		GameServer()->SendBroadcast(-1, false, _("Slaughter"));
		break;
	case 2:
		GameServer()->SendBroadcast(-1, false, _("Ocean of blood"));
		break;
	case 3:
		GameServer()->SendBroadcast(-1, false, _("Death takes all"));
		break;
	default:
		GameServer()->SendBroadcast(-1, false, _("Everybody dies"));
		break;
	};
}
...
bool IGameController::OnEntity(int Index, vec2 Pos)
{
	...
	*/
	...
	{
		new CTurret(&GameServer()->m_World, Pos + vec2(0, -10), TEAM_NEUTRAL, GameServer()->NewWeapon(GetModularWeapon(SW_GUN1, SW_GUN1)));
		return true;
	}
	else if (Index == ENTITY_TESLACOIL)
	{
